...
from agentspec.config import load_spec
from agentspec.run import run_spec


def test_buggy_agent_fails_contract() -> None:
    spec = load_spec("examples/support-agent/agentspec-buggy.yaml")
...
def test_fixed_agent_passes_contract() -> None:
    spec = load_spec("examples/support-agent/agentspec-fixed.yaml")
    report = run_spec(spec)
